...
export function Navigation() {
  ...
  return (
    <nav
      ...
      style={{ background: "linear-gradient(135deg, #0B3D2E 0%, #145A41 100%)" }}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-2">
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            {isAuthenticated ? (
              <>
                ...
                {isAdmin ? (
                  <Link to="/admin" className="text-white hover:text-[#FFD95A] transition-colors text-sm lg:text-base">
                    Admin
                  </Link>
                ) : null}
                <div className="flex items-center gap-3 border-l border-white/20 pl-6 ml-2">
                  <Button
                    size="sm"
                    variant="outline"
                    className="border-white/20 bg-transparent font-bold text-white hover:bg-white/10"
              ...
              <div className="flex items-center gap-3 border-l border-white/20 pl-6 ml-2">
                ...
                <Link to="/auth">
                  <Button
                    ...
                    style={{
                      boxShadow: "0 4px 14px 0 rgba(255, 217, 90, 0.39)",
                    }}
                  >
                    Get Started
                  </Button>
        ...
        {isOpen ? (
          <div className="md:hidden py-4 space-y-3 border-t border-white/10 mt-2">
            <Link to="/" className="block text-white hover:text-[#FFD95A] transition-colors py-2" onClick={() => setIsOpen(false)}>
              Home
            </Link>
            <Link to="/impact" className="block text-white hover:text-[#FFD95A] transition-colors py-2" onClick={() => setIsOpen(false)}>
              Impact
            </Link>
            <Link to="/pricing" className="block text-white hover:text-[#FFD95A] transition-colors py-2" onClick={() => setIsOpen(false)}>
              Pricing
            </Link>
            <Link to="/rewards" className="block text-white hover:text-[#FFD95A] transition-colors py-2" onClick={() => setIsOpen(false)}>
              Rewards
            </Link>
            {isAuthenticated ? (
              <>
                ...
                <div className="pt-2 flex flex-col gap-2">
                  <Button
                    variant="outline"
                    size="sm"
                    className="w-full font-bold border-white/20 text-white hover:bg-white/10"
                    onClick={() => void handleLogout()}
                  >
                    Log Out
                  </Button>
